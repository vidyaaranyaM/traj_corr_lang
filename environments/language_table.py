...
import environments.blocks as blocks_module
import environments.constants as constants
from environments.rewards import task_info
...
from environments.utils.utils_pybullet import add_visual_sphere
...
class LanguageTable(gym.Env):
  ...
  def __init__(self,
    ...
    self.render_target_position = (
        render_target_position if render_target_position is not None else np.array([0.35, 0.0, 0.0])
    )
    self.render_distance = render_distance
    self.render_yaw = render_yaw
    self.render_pitch = render_pitch
    self.place_visualizer()
  
  def place_visualizer(self) -> None:
      ...
      self._pybullet_client.resetDebugVisualizerCamera(
          cameraDistance=self.render_distance,
          cameraYaw=self.render_yaw,
          cameraPitch=self.render_pitch,
          cameraTargetPosition=self.render_target_position,
      )
  ...
  def _compute_observation(self, state=None):
    ...
    for k in self._get_urdf_paths().keys():
      if k in self._blocks_on_table:
        obs['block_%s_translation' % k] = state['block_%s_translation' % k]
        obs['block_%s_orientation' % k] = state['block_%s_orientation' % k]
    return obs
